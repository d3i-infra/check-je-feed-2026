...
- Keep the single canary assertion (`not df.empty`) in both shapes; no expected-column/dtype/nullability contracts in specs or expectation-map entries. A failing canary means "re-run the extractor with an `errors` Counter", not "read the diff from a schema".
- One cross-cutting content canary per platform is also allowed when it checks a shared helper's contract rather than a table's schema (e.g. every `Timestamp`/`Date`/`Created` value is ISO 8601, newest first). It skips without fixtures and names the failing table only as a pointer to inspection.
- A single-fixture platform lists `ExtractorSpec` entries (name, callable, kwargs) against `tests/ddp/<platform>_*.zip`. A multi-set/multi-account platform (several zip parts per export, fixtures from several accounts) instead discovers `tests/ddp/<platform>_set_*/` and pins per-set expectations in `EXPECT_NON_EMPTY: dict[str, set[str]]`, keyed by set name.
